...
class AME_ModelFixed(NamedTuple):
    """
    ...
    def __getitem__(self, key):
        ...
        return super(AME_ModelFixed, self).__getitem__(key)


class AME_Model(nn.Module):
    ...
    def __init__(self,
        ...
        if self.is_epdp:
            ...
            assert 0, "Unsupported problem: {}".format(problem.NAME)
        
        self.init_embed = nn.Linear(node_dim, embedding_dim)
        self.embedder = GraphAttentionEncoder_EDGE(
            n_heads=n_heads,
    ...
    def forward(self, input, return_pi=False):
        ...
        init_embedding = self._init_embed(input)
        ...
        mask = None
        # cost1, mask = self.problem.get_costs(input, pi)
    ...
    def _init_embed(self, input):
        ...
        if self.is_epdp:
            ...
            loc_all =torch.cat([input['loc'],input['depot'][:, None, :]],1)
            ...
            return torch.cat([embed_pick, embed_delivery, embed_depot], 1), edge_embedding
        
        else:
            assert 0, 'Gpn model only support EPDP problem'
    ...
    def _inner(self, input, embeddings):
        ...
        while not (self.shrink_size is None and state.all_finished()):
            ...
            # init_embedding = self._init_embed(input, state)
            # embeddings, _ = self.embedder(init_embedding)
            # Compute keys, values for the glimpse and keys for the logits once as they can be reused in every step
            # fixed = self._precompute(embeddings)
            ...
            i += 1
        state_cost = state.get_final_cost().squeeze(-1)
        # Collected lists, return Tensor
        return torch.stack(outputs, 1), torch.stack(sequences, 1), state_cost
    ...
    def _precompute(self, embeddings, num_steps=1):
        ...
        return AME_ModelFixed(embeddings, fixed_context, *fixed_attention_node_data)
    ...
    def _get_parallel_step_context(self, embeddings, state, from_depot=False):
        ...
        if self.is_epdp:
            ...
            if from_depot:
                ...
                return torch.cat(
                    (
                        ...
                        self.problem.BATTERY_CAPACITY - torch.zeros_like(state.used_capacity[:, :, None])
                    ),
                    -1
                )
            else:
            ...
            assert 0, 'GPN Only support EPDP'
