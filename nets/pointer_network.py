...
class Encoder(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        graph_size,_ , _ = x.size()
        ### Resort the node ordeR
        sort = [0] + list(range(1,graph_size-1,2)) + list(range(2,graph_size,2))
            
        return output[sort,: , :], hidden
...
class Decoder(nn.Module):
    ...
    def forward(self, state, decoder_input, embedded_inputs, hidden, context, eval_tours=None):
        """
        Args:
        ...
        return (torch.stack(outputs, 1), torch.stack(selections, 1)), hidden, costs
...
class PointerNetwork(nn.Module):
    ...
    def __init__(self,
        ...
        self.is_epdp = problem.NAME == 'epdp' or problem.NAME == 'sepdp' 
        ...
        if self.is_vrp or self.is_orienteering or self.is_pctsp:
            ...
            self.init_embed_delivery = nn.Linear(node_dim, embedding_dim)
            self.init_embed_station = nn.Linear(node_dim, embedding_dim)
            self.init_embed_depot = nn.Linear(node_dim, embedding_dim)
            # self.init_embed_depot = nn.Linear(6, embedding_dim)
            # self.W_placeholder = nn.Parameter(torch.Tensor(2 * embedding_dim))
            # self.W_placeholder.data.uniform_(-1, 1)  # Placeholder should be in range of activations
            # self.init_embed_pick = nn.Linear(node_dim * 2, embedding_dim)
            # self.init_embed_delivery = nn.Linear(node_dim, embedding_dim)
    ...
    def forward(self, inputs, eval_tours=None, return_pi=False):
        ...
        _log_p, pi, cost = self._inner(inputs, eval_tours)
    ...
    def _inner(self, inputs, eval_tours=None):
        ...
        state0 = self.problem.make_state(inputs)
        
        embedded_inputs = self._init_embed(inputs).transpose(1,0)

        encoder_hx = encoder_cx = Variable(
            torch.zeros(1, embedded_inputs.size(1), self.encoder.hidden_dim, out=embedded_inputs.data.new()),
            requires_grad=False
        )
        ...
        (pointer_probs, input_idxs), dec_hidden_t, costs = self.decoder(state0,
                                                                 decoder_input,
                                                                 embedded_inputs,
                                                                 dec_init_state,
                                                                 enc_h,
        ...
        return pointer_probs, input_idxs, costs
    ...
    def _init_embed(self, input):
        ...
        if self.is_pdp:
            ...
            embed_output = torch.cat([embed_depot, embed_pick, embed_delivery], 1)  
            
            
            sort = [0]
            for i in range(1,n_loc//2+1):
                sort = sort + [i] + [i+n_loc//2] 
            
            return embed_output[:, sort, :]
