...
class MultiHeadAttention_EDGE(nn.Module):
    def __init__(
        ...
        self.E_val1 = nn.Parameter(torch.Tensor(n_heads, input_dim, val_dim))
        self.E_val2 = nn.Parameter(torch.Tensor(n_heads, val_dim, 1))
    ...
    def forward(self, input, h=None, mask=None):
        ...
        if e is not None:
            ...
            E = torch.matmul(torch.matmul(eflat,self.E_val1),self.E_val2).view(shp_e)
        ...
        compatibility = self.norm_factor * (torch.matmul(Q, K.transpose(2, 3)) + E.squeeze(-1))
        ...
        heads = torch.matmul(attn, V) + edge
