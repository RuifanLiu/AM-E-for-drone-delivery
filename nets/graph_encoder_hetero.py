...
class MultiHeadAttention_HETERO(nn.Module):
    def __init__(
            self,
            n_heads,
            input_dim,
        ...
        super(MultiHeadAttention_HETERO, self).__init__()
        ...
        self.W8_query = nn.Parameter(torch.Tensor(n_heads, input_dim, key_dim))
    ...
    def forward(self, q, h=None, mask=None):
        ...
        n_station = 3
        n_pick = (graph_size - 1 - n_station) // 2
        ...
        shp_q_station = (self.n_heads, batch_size, n_pick, -1)
        shp_station = (self.n_heads, batch_size, n_station, -1)
        ...
        delivery_flat = h[:, n_pick:2*n_pick, :].contiguous().view(-1, input_dim)  # [batch_size * n_pick, embed_dim]
        station_flat = h[:, 2*n_pick:-1, :].contiguous().view(-1, input_dim)  # [batch_size * n_pick, embed_dim]
        ...
        V_station = torch.matmul(station_flat, self.W_val).view(shp_station)
        
        
        # pickup -> its delivery
        V_additional_delivery = torch.cat([  # [n_heads, batch_size, graph_size, key_size]
            V_delivery,  # [n_heads, batch_size, n_pick, key/val_size]
            torch.zeros(self.n_heads, batch_size, n_pick + n_station + 1, self.input_dim // self.n_heads, dtype=V.dtype, device=V.device)
            ], 2)
        ...
        Q_delivery_allpickup = torch.matmul(delivery_flat, self.W6_query).view(shp_alldelivery)  # (self.n_heads, batch_size, n_pick, key_size)
        K_allpickup2 = torch.matmul(pick_flat, self.W_key).view(shp_q_alldelivery)  # (self.n_heads, batch_size, n_pick, -1)
        V_allpickup2 = torch.matmul(pick_flat, self.W_val).view(shp_q_alldelivery)

        # delivery -> station
        Q_delivery_station = torch.matmul(delivery_flat, self.W8_query).view(shp_q_station)  # (self.n_heads, batch_size, n_pick, key_size)
        K_station = torch.matmul(station_flat, self.W_key).view(shp_station)
        V_station = torch.matmul(station_flat, self.W_val).view(shp_station)
...
#            ], 2)        
        V_additional_pick = torch.cat([  # [n_heads, batch_size, graph_size, key_size]
            ...
            torch.zeros(self.n_heads, batch_size, n_station + 1, self.input_dim // self.n_heads, dtype=V.dtype, device=V.device),
            ], 2)  
        ...
        compatibility_pick_station = self.norm_factor * torch.matmul(Q_pick_station, K_station.transpose(2, 3))  # [n_heads, batch_size, n_pick, n_pick]
        ...
        compatibility_delivery_station = self.norm_factor * torch.matmul(Q_delivery_station, K_station.transpose(2, 3))  # [n_heads, batch_size, n_pick, n_pick]
        ...
        compatibility_additional_delivery = torch.cat([  # [n_heads, batch_size, graph_size, 1]
            ...
            -np.inf * torch.ones(self.n_heads, batch_size, n_pick + n_station + 1, dtype=compatibility.dtype, device=compatibility.device)
        ...
        compatibility_additional_allpick = torch.cat([  # [n_heads, batch_size, graph_size, n_pick]
            ...
            -np.inf * torch.ones(self.n_heads, batch_size, n_pick + n_station + 1,n_pick,  dtype=compatibility.dtype, device=compatibility.device)
        ...
        compatibility_additional_station = torch.cat([  # [n_heads, batch_size, graph_size, n_pick]
            compatibility_pick_station,  # [n_heads, batch_size, n_pick, n_pick]
            -np.inf * torch.ones(self.n_heads, batch_size, n_pick + n_station + 1, n_station, dtype=compatibility.dtype, device=compatibility.device)
        ], 2).view(self.n_heads, batch_size, graph_size, n_station)
        # [n_heads, batch_size, n_query, graph_size+1+n_pick+n_pick]
        ...
        compatibility_additional_pick = torch.cat([  # [n_heads, batch_size, graph_size, 1]
            ...
            -np.inf * torch.ones(self.n_heads, batch_size, n_station + 1, dtype=compatibility.dtype, device=compatibility.device)
            ], -1).view(self.n_heads, batch_size, graph_size, 1)        
        ...
        compatibility_additional_alldelivery2 = torch.cat([  # [n_heads, batch_size, graph_size, n_pick]
            ...
            -np.inf * torch.ones(self.n_heads, batch_size, n_station + 1, n_pick, dtype=compatibility.dtype, device=compatibility.device)
            ], 2).view(self.n_heads, batch_size, graph_size, n_pick)    
        ...
        compatibility_additional_station2 = torch.cat([  # [n_heads, batch_size, graph_size, n_pick]
            -np.inf * torch.ones(self.n_heads, batch_size, n_pick, n_station, dtype=compatibility.dtype, device=compatibility.device),
            compatibility_delivery_station,  # [n_heads, batch_size, n_pick, n_pick]
            -np.inf * torch.ones(self.n_heads, batch_size, n_station + 1, n_station, dtype=compatibility.dtype, device=compatibility.device)
            ], 2).view(self.n_heads, batch_size, graph_size, n_station)
        ...
        compatibility = torch.cat([compatibility, compatibility_additional_delivery, compatibility_additional_allpick, compatibility_additional_alldelivery,
                                   compatibility_additional_station,
                                   compatibility_additional_pick, compatibility_additional_alldelivery2, compatibility_additional_allpick2,
                                   compatibility_additional_station2], dim=-1)
        ...
        heads = heads + torch.matmul(attn[:, :, :, graph_size+1+2*n_pick :graph_size*2].view(self.n_heads, batch_size, graph_size, n_station), V_station)

        # delivery
        heads = heads + attn[:, :, :, graph_size*2].view(self.n_heads, batch_size, graph_size, 1) * V_additional_pick 
        
        heads = heads + torch.matmul(attn[:, :, :, graph_size*2 + 1:graph_size*2 + 1 + n_pick].view(self.n_heads, batch_size, graph_size, n_pick), V_alldelivery2)
        
        heads = heads + torch.matmul(attn[:, :, :, graph_size*2 + 1 + n_pick : graph_size*2 + 1 + n_pick*2].view(self.n_heads, batch_size, graph_size, n_pick), V_allpickup2)

        heads = heads + torch.matmul(attn[:, :, :, graph_size*2 + 1 + n_pick*2:].view(self.n_heads, batch_size, graph_size, n_station), V_station)
...
class GraphAttentionEncoder_HETERO(nn.Module):
    def __init__(
            self,
            n_heads,
            embed_dim,
        ...
        super(GraphAttentionEncoder_HETERO, self).__init__()
