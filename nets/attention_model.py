...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        if self.attention_type == 'Kool':
            ...
            self.init_embed_edge = nn.Linear(1, embedding_dim)
            self.embedder = GraphAttentionEncoder_EGAT(
                n_heads=n_heads,
                embed_dim=embedding_dim,
                n_layers=self.n_encode_layers,
                normalization=normalization
            ...
            self.embedder = GraphAttentionEncoder_HETERO(
                n_heads=n_heads,
                embed_dim=embedding_dim,
                n_layers=self.n_encode_layers,
                normalization=normalization
            ...
            assert 0, "Unsupported attention typr: {}".format(self.attention_type)
    ...
    def forward(self, input, return_pi=False):
        ...
        _log_p, pi, cost, _ = self._inner(input, embeddings)
        mask = None
    ...
    def _inner(self, input, embeddings):
        ...
        fail_rate = state.get_failure_rate().squeeze(-1)
        # Collected lists, return Tensor
        return torch.stack(outputs, 1), torch.stack(sequences, 1), costs, fail_rate
    ...
    def sample_many(self, input, batch_rep=1, iter_rep=1):
        ...
        return sample_many(
            ...
            lambda input, pi: self.problem.get_failure_func(input[0], pi),  # Don't need embeddings as input to get_costs
